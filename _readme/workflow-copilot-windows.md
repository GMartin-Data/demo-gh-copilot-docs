# Workflow Copilot Agent Mode — Environnement Windows
...
|---|-----|--------|---------------|
| 11 | DEV | `. .\load_env.ps1` (terminal PowerShell intégré VS Code) | 5 sec |
...
- **`. .\load_env.ps1`** doit être exécuté par DEV avant de lancer Copilot — l'agent ne doit pas accéder à `.env` directement. Le point initial (dot-sourcing) est obligatoire : sans lui, les variables restent dans un sous-processus et ne sont pas visibles par dbt.
...
- **PowerShell 5.1** : le terminal par défaut sur la machine entreprise est PS 5.1. Vérifier que le venv dbt s'active correctement avec `.venv\Scripts\Activate.ps1` avant de lancer `load_env.ps1`.
...
- [ ] `.env` renseigné
- [ ] `. .\load_env.ps1` exécuté (terminal PowerShell intégré VS Code)
